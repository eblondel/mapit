...
create_flow_map <- function(sf, by, data, data.col.from, data.col.to, 
                            ...
                            labels = TRUE, label.box.col = "#0088A2", label.bg = "#C3E4E5"){
  ...
  for(i in 1:nrow(data)){
    ...
    if(labels) mapit::create_legend(sf.from.coords[1], sf.from.coords[2], paste0(data[i,]$value*100,"%"), text.col = "white", text.font = 2, 
  ...
  for(i in 1:nrow(data)){
    if(data[i,][,data.col.from] == data[i,][,data.col.to]){
      ...
      if(labels) mapit::create_legend(sf.to.coords[1], sf.to.coords[2], paste0(data[i,]$value*100,"%"), text.col = "white", text.font = 2, 
