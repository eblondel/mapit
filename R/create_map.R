...
create_map <- function(sf = NULL, sfby = NULL, sfby.code = NULL, bbox = NULL,
  ...
  #base continent
  if(!add){
    continent_layer <- mapit::un_continent_lowres_eck4
    if(!is.null(sfby)) if(startsWith(sfby,"countries")){
      continent_layer = mapit::un_continent_nopole_lowres_eck4
  ...
  if(!is.null(sfby)) if(startsWith(sfby, "countries")) if(!add){
    ...
    switch(sfby,
      "countries" = {
        if(displayWaterboundaries) plot(mapit::un_water_bodies_eck4, col = waterbCol, border = "transparent", add = TRUE) 
        boundaries = mapit::un_boundaries_eck4
      ...
      "countries_lowres" = {
        if(displayWaterboundaries) plot(mapit::un_water_bodies_eck4, col = waterbCol, border = "transparent", add = TRUE)
        boundaries = mapit::un_boundaries_lowres_eck4
        plot(boundaries[boundaries$TYPE == 1,][1], lwd = 0.45, col = boundCol, lty = "812121", add = TRUE)
        plot(boundaries[boundaries$TYPE == 2,][1], lwd = 0.35, col = boundCol, lty = "21", add = TRUE)
        plot(boundaries[boundaries$TYPE == 3,][1], lwd = 0.35, col = boundCol, lty = "21", add = TRUE)
  ...
  if(maptype == "choropleth"){
    ...
    if(!is.null(sfby)) if(sfby=="countries_lowres"){
      ...
      if(add_small_features_as_dots) plot(sf::st_point_on_surface(small.sf)[1], border="transparent", pch = 21, cex = small_features_dots_cex, col = small.sf$col, bg = "transparent", add = TRUE)
  ...
  if(faoareas){
    fao_areas_lines <- mapit::fao_areas_lines_eck4
